...
function isCompressible(type: string) {
    return /(jpeg|png)$/i.test(type);
}

function urlParamToString(urlParam: any) {
    ...
    needEncodedParam.forEach((key: string) => {
        if (urlParam[key]) {
 ...
 */
function toBase64(file: Blob, option: any) {
    const baseOption = {
        orientation: true,
        canvas: true,
        quality: 0.8
    ...
    return new Promise((resolve, reject) => {
        if (isCompressible(file.type)) {
            option = _.extend(baseOption, option);
            loadImage(
                file,
                (canvas: any) => {
 ...
 */
function upload(param: any) {
    const baseURL = param.baseURL;
    const token = param.token;
    const fileBase64 = param.base64;
    const xhr = new XMLHttpRequest();
    const baseURLParam = {
        putb64: '-1'
    };
    const urlParam = _.extend(param.urlParam, baseURLParam);
    const uriObj = new Uri(baseURL);
    if (!uriObj.protocol()) {
        uriObj.setProtocol(window.location.protocol);
    }
    const postURL = uriObj.setPath(urlParamToString(urlParam)).toString();
    return new Promise((resolve, reject) => {
        xhr.onload = () => {
